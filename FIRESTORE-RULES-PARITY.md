...
| `examSessions` read/write | `if false` (holds the answer key)                      | no route returns a session; key read only to score                        | exams §"answer key never reaches the client" | ✅ E1, E3                  |
| `examAttempts` write      | server-only                                            | scored server-side against the stored key; client score ignored           | exams §"scoring is server-side"              | ✅                         |
| `examAttempts` read       | owner-or-admin                                         | throttle counts own attempts; verify-on-pass audited                      | exams §throttle                              | ✅ E2                      |
...
| `publicAccountants`                | **superseded** — `GET /v1/accountants` returns safe fields via serialise-on-read; no mirror collection, no sync trigger, no drift                                                                                                                                                                             | Phase 4                                              |
