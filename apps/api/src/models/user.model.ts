...
const schema = new mongoose.Schema<UserDocument>(
  {
    // Unique: one user row per auth identity. Without the constraint, a
    ...
    // Indexed (non-unique — phone is optional and historically unconstrained):
    // OTP verification resolves phone → authUid on every sign-in
    // (`findAuthUidByPhone`), which must not scan the collection.
