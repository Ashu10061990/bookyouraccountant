...
import { ExamAttemptModel, ExamSessionModel } from "../models/exam.model.js";
...
describe("passing the exam verifies the accountant", () => {
  it("marks an existing profile verified, audited, on a pass", async () => {
    ...
    const acc = await AccountantModel.findOne({ authUid: UIDS.accountant });
