...
describe("POST /v1/businesses", () => {
  it("creates the caller's own profile", async () => {
    ...
    expect(res.json()).toMatchObject({
      business: { authUid: UIDS.business, orgName: "Iyer Traders", freeDayCouponUsed: false },
    });
  });
  ...
  it("ignores a authUid in the body", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/v1/businesses",
      headers: as(TOKENS.business),
      payload: { ...PROFILE, authUid: UIDS.other },
    });

    expect(res.statusCode).toBe(403);
    expect(await BusinessModel.countDocuments({ authUid: UIDS.other })).toBe(0);
  });
...
describe("the organisation PAN", () => {
  ...
  it("is stored encrypted and returned masked", async () => {
    ...
    const stored = await BusinessModel.findOne({ authUid: UIDS.business }).lean();
  ...
  it("cannot be set or cleared through the profile update path", async () => {
    ...
    const stored = await BusinessModel.findOne({ authUid: UIDS.business });
