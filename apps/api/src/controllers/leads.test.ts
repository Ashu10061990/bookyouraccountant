...
describe("PUT /v1/leads/me", () => {
  it("captures a lead for the caller", async () => {
    ...
    expect(res.json()).toMatchObject({
      lead: { authUid: UIDS.business, completed: false, mode: "register" },
    });
  });
  ...
  it("works before the user has any user record", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(await UserModel.countDocuments({ authUid: UIDS.stranger })).toBe(0);
  });
  ...
  it("ignores a authUid in the body and keys the lead to the token", async () => {
    await app.inject({
      method: "PUT",
      url: "/v1/leads/me",
      headers: as(TOKENS.business),
      payload: { authUid: UIDS.other, phone: "+919876543210" },
    });

    expect(await LeadModel.countDocuments({ authUid: UIDS.business })).toBe(1);
    expect(await LeadModel.countDocuments({ authUid: UIDS.other })).toBe(0);
  });
...
describe("GET /v1/leads/me", () => {
  it("returns only the caller's own lead", async () => {
    await LeadModel.create([
      ...
      { authUid: UIDS.other, mode: "register", completed: true, lastSeenAt: new Date() },
    ]);
