...
describe("POST /v1/users", () => {
  it("creates the caller's own record from a valid token", async () => {
    ...
    expect(res.json()).toMatchObject({
      user: { authUid: UIDS.business, role: "business", blocked: false },
    });
  });
  ...
  it("ignores a authUid in the body and keys the record to the token", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/v1/users",
      headers: as(TOKENS.business),
      payload: { role: "business", authUid: UIDS.other },
    });

    expect(res.statusCode).toBe(201);
    expect(res.json()).toMatchObject({ user: { authUid: UIDS.business } });
    expect(await UserModel.findOne({ authUid: UIDS.other })).toBeNull();
  });
  ...
  it("ignores a blocked flag in the body", async () => {
    ...
    const stored = await UserModel.findOne({ authUid: UIDS.business });
  ...
  it("rejects a second record for the same identity with a 409", async () => {
    ...
    expect(await UserModel.countDocuments({ authUid: UIDS.business })).toBe(1);
  });
...
describe("GET /v1/users/me", () => {
  beforeEach(async () => {
    await UserModel.create({ authUid: UIDS.business, role: "business", blocked: false });
  });
  ...
  it("rejects a blocked user", async () => {
    await UserModel.create({ authUid: UIDS.other, role: "business", blocked: true });
...
describe("PATCH /v1/users/me", () => {
  ...
  it("cannot unblock itself", async () => {
    await UserModel.updateOne({ authUid: UIDS.business }, { $set: { blocked: true } });
    ...
    expect(res.statusCode).toBe(403);
    expect((await UserModel.findOne({ authUid: UIDS.business }))?.blocked).toBe(true);
  });

  it("cannot change its own authUid to take over another identity", async () => {
    await UserModel.create({ authUid: UIDS.other, role: "business", blocked: false });

    await app.inject({
      method: "PATCH",
      url: "/v1/users/me",
      headers: as(TOKENS.business),
      payload: { authUid: UIDS.other },
    });

    // Both records still keyed to their own identity.
    expect(await UserModel.countDocuments({ authUid: UIDS.business })).toBe(1);
    expect(await UserModel.countDocuments({ authUid: UIDS.other })).toBe(1);
  });
});

describe("the authUid unique index", () => {
  ...
  it("rejects a duplicate identity at the database level", async () => {
    ...
    await expect(
      UserModel.create({ authUid: UIDS.business, role: "accountant", blocked: false }),
