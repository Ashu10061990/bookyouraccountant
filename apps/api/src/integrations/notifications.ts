 ...
 * one `Notifier`, mirroring `helpers/crypto.helper.ts`'s `Cipher` and
 * `integrations/storage.ts`'s `StoragePort`: a narrow interface, real adapters
 ...
 * (`repositories/notifications.repository.ts`) is wired in at
...
declare module "fastify" {
  interface FastifyInstance {
     ...
     * `storage` is in `integrations/storage.ts`.
     */
    notifier: Notifier;
  }
}
...
> = {
  // Sign-in OTP — SMS only, MSG91 being the primary auth channel. No email
  // template on purpose: the flow is keyed to a phone, and a code that could
  // also land in an inbox would widen the attack surface for no benefit.
  auth_otp: {
    sms: {
      body: "{otp} is your BookYourAccountant sign-in code. It expires in {minutes} minutes. Never share it.",
    },
  },
  accountant_verified: {
    email: {
