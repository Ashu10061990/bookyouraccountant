...
import type { SealedField } from "../models/accountant.model.js";
import { BusinessModel, type BusinessDocument } from "../models/business.model.js";

/** Data access for businesses. The only file in this module importing Mongoose. */

export type NewBusiness = Pick<
  BusinessDocument,
  "authUid" | "fullName" | "signatoryRole" | "orgName" | "orgType" | "address"
> & {
...
export async function findByUid(authUid: string): Promise<BusinessDocument | null> {
  return BusinessModel.findOne({ authUid }).lean<BusinessDocument | null>().exec();
}
...
export async function updateProfile(
  ...
  return BusinessModel.findOneAndUpdate({ authUid }, { $set: changes }, { new: true })
