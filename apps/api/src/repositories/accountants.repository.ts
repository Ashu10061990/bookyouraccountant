import { DEFAULT_LIST_LIMIT, MAX_LIST_LIMIT } from "../constants/index.js";
import type { ClientSession } from "mongoose";
import {
  AccountantModel,
  type AccountantDocument,
  type AccountantKyc,
} from "../models/accountant.model.js";
...
export async function findByUid(authUid: string): Promise<AccountantDocument | null> {
  return AccountantModel.findOne({ authUid }).lean<AccountantDocument | null>().exec();
}
...
export async function updateProfile(
  ...
  return AccountantModel.findOneAndUpdate({ authUid }, { $set: changes }, { new: true })
