...
import { LeadModel, type LeadDocument } from "./leads.schema.js";
 ...
 * with it.
 */
export async function upsertOwn(firebaseUid: string, input: LeadUpsertInput): Promise<LeadView> {
  const updated = await LeadModel.findOneAndUpdate(
    { firebaseUid },
    { $set: { ...input, lastSeenAt: new Date() }, $setOnInsert: { firebaseUid } },
...
export async function findOwn(firebaseUid: string): Promise<LeadView | null> {
  const document = await LeadModel.findOne({ firebaseUid }).lean<LeadDocument | null>().exec();
