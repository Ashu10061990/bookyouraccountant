...
import { AccountantModel } from "./accountants.schema.js";
...
describe("POST /v1/accountants", () => {
  it("creates the caller's own profile", async () => {
    ...
    expect(res.json()).toMatchObject({
      accountant: { firebaseUid: UIDS.accountant, name: "Asha Menon", verified: false },
    });
  });
  ...
  it.each([
    ...
    ["firebaseUid", { firebaseUid: UIDS.other }],
...
describe("PATCH /v1/accountants/me", () => {
  ...
  it.each([
    ...
    const stored = await AccountantModel.findOne({ firebaseUid: UIDS.accountant });
...
describe("KYC storage", () => {
  ...
  it("stores the PAN encrypted, never in plaintext", async () => {
    ...
    const stored = await AccountantModel.findOne({ firebaseUid: UIDS.accountant }).lean();
...
describe("when no KMS key is configured", () => {
  it("fails loudly rather than storing KYC in plaintext", async () => {
    ...
    try {
      ...
      const stored = await AccountantModel.findOne({ firebaseUid: UIDS.stranger }).lean();
