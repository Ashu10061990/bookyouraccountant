...
import type { UserView } from "./users.repository.js";
 ...
 *
 * `firebaseUid` is a parameter, not a body field: identity comes from the
 * verified token. A uid in the request body is ignored entirely.
 */
export async function createUser(firebaseUid: string, input: CreateUserInput): Promise<UserView> {
  assertSelfAssignable(input.role);

  const existing = await repository.findByFirebaseUid(firebaseUid);
  if (existing !== null) {
    throw conflict("This account already exists.");
  }

  return repository.insert({ firebaseUid, ...input });
}

export async function getOwnUser(firebaseUid: string): Promise<UserView> {
...
export async function updateOwnUser(
  ...
  const updated = await repository.updateByFirebaseUid(firebaseUid, changes);
...
export function recordLogin(firebaseUid: string): Promise<void> {
  return repository.touchLastLogin(firebaseUid);
}
